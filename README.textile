...
Main interface for managing the billing and payment for an account.
...
The Promotion is a modification to the RatePlan which provides information on when, who, pricing modifications and code used in creating and modifying a Purchase.

A Purchase combines a RatePlan + Promotions with a Subscription to create an an actionable entity for which Payments can be made. The Purchase contains the TimePeriod for which the Subscription is valid and information about the payer (name, address). It also details the type of payments allowed (credit card, check, PO).
